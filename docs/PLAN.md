...
| 9 | Printer? | **Settled 17 Aug 2026: HP Smart Tank 580 (1F3Y2A)** — A4, Wi-Fi 802.11b/g/n + Wi-Fi Direct, Mopria-certified. Small printer still planned | **Nothing to buy and nothing to build**, but two corrections to the 16 Aug note: it has **no Ethernet port** (Wi-Fi or USB only, so there is no wired fallback if the Wi-Fi is down), and its Wi-Fi is **2.4 GHz only** — on a dual-band router the tablets can sit on 5 GHz and lose mDNS discovery, which looks exactly like a broken printer. Its Bluetooth is HP Smart app onboarding, not a print transport. Residual steps: same 2.4 GHz network with AP isolation off, and HP Print Service Plugin on both tablets during the M0 §1.3 setup (`TABLET.md` §1) |
...
| ~~His printer cannot be reached from a tablet~~ | **Low — resolved 17 Aug 2026.** It is an HP Smart Tank 580: Wi-Fi and Mopria-certified, so the tablets reach it. No Ethernet, and 2.4 GHz only (`TABLET.md` §1) | — | Residual, both at M0 §1.3: HP Print Service Plugin on both tablets, and printer plus tablets on the same 2.4 GHz network with AP isolation off — a dual-band router can break mDNS discovery while everything still pings. Then print one real sheet before go-live |
