...
export function MedicinesPanel({ chrome }: PanelProps) {
  const session = currentSession();
  const allowed = session?.role === 'admin' || session?.role === 'counter';
  ...
  if (!allowed) {
    return (
      <ThreePane tabs={chrome}
        context={<div />}
      >
        <PageHeader eyebrow="Pharmacy" title="Medicines" />
        <Notice tone="bad">The medicine list is kept by the pharmacy or an administrator.</Notice>
      </ThreePane>
    );
  }
  ...
  return (
    ...
    >
      <PageHeader eyebrow="Pharmacy" title="Medicines" sub={session?.staffName} />
