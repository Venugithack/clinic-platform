...
import { parse } from 'npm:csv-parse@6.1.0/sync'
import { hasRole } from '../_shared/auth.ts'
import { audit, db, isoNow, transaction } from '../_shared/db.ts'
import { cors, json, preflight, sessionFrom } from '../_shared/http.ts'
import type { SaleClass } from '../_shared/types.ts'
...
async function handleGet(request: Request) {
  ...
  if (!session || !hasRole(session, 'admin', 'pharmacy')) {
    return json({ ok: false, message: 'Inventory access required.' }, 401)
  }
...
async function handlePost(request: Request) {
  ...
  try {
    ...
    return json({ ok: true, message: `${rows.length} inventory row(s) imported.` })
  } catch (error) {
...
/**
 * Inventory as a spreadsheet, in and out.
 *
 * Next routed GET and POST to two exported functions; a Deno function gets one
 * handler and dispatches itself. The bodies below are unchanged from the route
 * this replaces, apart from where the session comes from.
 */
