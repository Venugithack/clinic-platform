...
| 9 | Printer? | **Settled 16 Aug 2026: A4, Bluetooth *and* Wi-Fi/Ethernet.** Small printer still planned | **Nothing to buy and nothing to build.** The tablets reach it over the clinic Wi-Fi and the Bluetooth radio is ignored — Bluetooth would not have worked (`TABLET.md` §1). One residual step: Android only sees a network printer through Mopria or the manufacturer's print service plugin, so that plugin gets installed on both tablets during the M0 §1.3 tablet setup |
...
| ~~His printer cannot be reached from a tablet~~ | **Low — largely resolved 16 Aug 2026.** The printer is Bluetooth *and* Wi-Fi/Ethernet, so it is reachable over the LAN. Bluetooth alone would have been unusable (`TABLET.md` §1) | — | Residual: Android reaches a network printer only through Mopria or the manufacturer's print service plugin. Install it on both tablets during M0 §1.3, and print one real sheet before go-live |
